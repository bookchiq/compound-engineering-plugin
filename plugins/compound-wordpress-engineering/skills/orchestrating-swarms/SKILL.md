...
Task({
  subagent_type: "compound-wordpress-engineering:review:wp-php-reviewer",
  description: "WordPress PHP review",
  prompt: "Review this WordPress PHP code for coding standards and security"
})
...
**All review agents from compound-wordpress-engineering:**
...
- `schema-drift-detector` - Database schema drift detection
- `security-sentinel` - Security vulnerabilities
- `wp-frontend-races-reviewer` - JavaScript/Interactivity API race conditions
- `wp-gutenberg-reviewer` - Block editor patterns and deprecations
- `wp-hooks-reviewer` - WordPress hook system architecture
...
Task({
  ...
  prompt: "Research current best practices for WordPress REST API authentication 2024-2026"
})

// Framework documentation
Task({
  subagent_type: "compound-engineering:research:framework-docs-researcher",
  description: "Research WordPress media handling",
  prompt: "Gather comprehensive documentation about WordPress media uploads and attachment handling"
})
...
const research = await Task({
  ...
  prompt: "Research best practices for implementing caching in WordPress. Include: object cache, transients, cache invalidation strategies, cache key design."
})
...
// Good
prompt: `
  1. Review custom queries for N+1 patterns
  2. Check all WP_Query calls have proper meta_query optimization
