...
- Foundational requirement (WordPress core, REST API, hooks, etc.)
...
2. **Gather context:**
   ...
   - Symptom: Archive page taking >5 seconds, N+1 query when loading post meta
   - Failed attempts: Added pagination (didn't help), checked WP_Query args
   - Solution: Added `update_post_meta_cache => true` and batch meta query
   - Root cause: Individual get_post_meta() calls in loop causing separate query per post
3. **Check existing:** No similar issue found
4. **Generate filename:** `n-plus-one-post-meta-CustomPostType-20251110.md`
5. **Validate YAML:**
   ```yaml
   module: Custom Post Type
   date: 2025-11-10
   ...
   component: wp_plugin
   symptoms:
     - "N+1 query when loading post meta"
     - "Archive page taking >5 seconds"
   root_cause: missing_include
   severity: high
   tags: [n-plus-one, meta-cache, performance]
   ```
   ✅ Valid
6. **Create documentation:**
   - `docs/solutions/performance-issues/n-plus-one-post-meta-CustomPostType-20251110.md`
