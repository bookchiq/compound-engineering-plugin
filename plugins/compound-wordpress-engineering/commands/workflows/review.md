...
**DATABASE: If PR contains dbDelta calls, $wpdb->query CREATE/ALTER TABLE, or activation hooks with schema changes:**

- Task schema-drift-detector(PR content) - Detects unrelated database schema changes by cross-referencing dbDelta calls and version tracking (run FIRST)
...
**When to run:**
- PR includes `dbDelta()` calls or `$wpdb->query("CREATE TABLE` / `ALTER TABLE` patterns
- PR modifies plugin activation hooks with database operations
- PR includes data backfill or migration scripts
- PR title/body mentions: migration, backfill, data transformation, custom table, dbDelta
...
**Tagging:** Always add `code-review` tag, plus: `security`, `performance`, `architecture`, `wordpress`, `php`, `gutenberg`, `quality`, etc.
