# Compound WordPress Engineering - Claude Code Plugin Marketplace

This repository is a Claude Code plugin marketplace that distributes the `compound-wordpress-engineering` plugin — a WordPress-focused adaptation of [EveryInc/compound-engineering-plugin](https://github.com/EveryInc/compound-engineering-plugin). It provides AI-powered development tools specialized for WordPress theme, plugin, and block development.
...
└── plugins/
    └── compound-wordpress-engineering/   # The WordPress-focused plugin
        ├── .claude-plugin/
...
```bash
# Count agents
ls plugins/compound-wordpress-engineering/agents/*.md | wc -l

# Count commands
ls plugins/compound-wordpress-engineering/commands/*.md | wc -l

# Count skills
ls -d plugins/compound-wordpress-engineering/skills/*/ 2>/dev/null | wc -l
```
...
- [ ] `plugins/compound-wordpress-engineering/.claude-plugin/plugin.json` → `version`
- [ ] `.claude-plugin/marketplace.json` → plugin `version`

#### 4. Update documentation

- [ ] `plugins/compound-wordpress-engineering/README.md` → list all components
- [ ] `plugins/compound-wordpress-engineering/CHANGELOG.md` → document changes
...
cat plugins/compound-wordpress-engineering/.claude-plugin/plugin.json | jq .
```
...
grep -o "Includes [0-9]* specialized agents" plugins/compound-wordpress-engineering/.claude-plugin/plugin.json
ls plugins/compound-wordpress-engineering/agents/*.md | wc -l
```
...
1. Create `plugins/compound-wordpress-engineering/agents/new-agent.md`
...
1. Create `plugins/compound-wordpress-engineering/commands/new-command.md`
...
1. Create skill directory: `plugins/compound-wordpress-engineering/skills/skill-name/`
